...
ClientProxy1_5::sendDragInfo(UInt32 fileCount, const char* info, size_t size)
{
	...
	LOG((CLOG_NOTE "CLINET1_5::SENDDRAGINFO"));
...
ClientProxy1_5::fileChunkSending(UInt8 mark, char* data, size_t dataSize)
{
	...
	switch (mark) {
	case kFileStart: //파일 연결준비
		LOG((CLOG_NOTE "file sending start: size=%s", data));
		break;
		
	case kFileChunk: // 파일 전송중
		LOG((CLOG_NOTE "file chunk sending: size=%i", chunk.size()));
		break;

	case kFileEnd: //파일 전송 끝
		LOG((CLOG_NOTE "file sending finished"));
		break;
	}
...
ClientProxy1_5::fileChunkReceived()
{
	...
	switch (mark) {
	case kFileStart: // 파일 받을 준비 
		server->clearReceivedFileData();
		server->setExpectedFileSize(content);
		if (CLOG->getFilter() >= kDEBUG2) {
			LOG((CLOG_NOTE "recv file data from client: file size=%s", content.c_str()));
			m_stopwatch.start();
		}
		break;

	case kFileChunk: // 파일 받는 중
		server->fileChunkReceived(content);
		if (CLOG->getFilter() >= kDEBUG2) {
				LOG((CLOG_NOTE "recv file data from client: chunck size=%i", content.size()));
				double interval = m_stopwatch.getTime();
				m_receivedDataSize += content.size();
				LOG((CLOG_NOTE "recv file data from client: interval=%f s", interval));
				if (interval >= m_intervalThreshold) {
					double averageSpeed = m_receivedDataSize / interval / 1000;
					LOG((CLOG_NOTE "recv file data from client: average speed=%f kb/s", averageSpeed));
