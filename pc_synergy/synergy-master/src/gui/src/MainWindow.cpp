...
void MainWindow::createMenuBar()
{
	...
	m_pMenuWindow->addAction(m_pActionRestore);
        ...
        TitleShell * centralWidget = new TitleShell(this);
        ...
        vBox->addWidget(m_pMenuBar);
        setMenuWidget(centralWidget);
	...
	if (desktopMode)
	{
		if (!appConfig().startedBefore()) {
			QMessageBox::information(
                                this, "OS Jumper",
                                tr("OS Jumper will be minimized to the notification "
...
bool MainWindow::serverArgs(QStringList& args, QString& app)
{
	...
	if (!QFile::exists(app))
	{
                QMessageBox::warning(this, tr("OS Jumper server not found"),
...
void MainWindow::stopDesktop()
{
	if (!synergyProcess()) {
        ...
        appendLogNote("stopping OS Jumper desktop process");
...
void MainWindow::synergyFinished(int exitCode, QProcess::ExitStatus)
{
	...
	if (exitCode != 0)
	{
                QMessageBox::critical(this, tr("OS Jumper terminated with an error"), QString(tr("OS Jumper terminated unexpectedly with an exit code of %1.<br><br>Please see the log output for details.")).arg(exitCode));
		stopSynergy();
	}
#else
	Q_UNUSED(exitCode);
...
void MainWindow::setSynergyState(qSynergyState state)
{
	...
	case synergyConnected: {
                setStatus(tr("OS Jumper is running."));
		break;
	}
	case synergyConnecting:
                setStatus(tr("OS Jumper is starting."));
		break;
	case synergyDisconnected:
                setStatus(tr("OS Jumper is not running."));
		break;
	}
