...
ServerProxy::parseMessage(const UInt8* code)
{
	if (memcmp(code, kMsgDMouseMove, 4) == 0) {
		...
		LOG((CLOG_DEBUG1 "DEBUG[SERVERPROXY::PARESEMESSAGE()]"));
		leave();
	}
...
{
	// parse
	//LOG((CLOG_NOTE "DEBUG[SERVERPROXY::LEAVE()]"));
...
ServerProxy::fileChunkReceived()
{
	...
	case kFileChunk:
		//LOG((CLOG_NOTE "ServerProxy::fileChunkReceived[1]"));
...
ServerProxy::fileChunkSending(UInt8 mark, char* data, size_t dataSize)
{
	...
	//LOG((CLOG_NOTE "ServerProxy::fileChunkSending"));
