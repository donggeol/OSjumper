...
{
	assert(m_enabled);
	//LOG((CLOG_NOTE "DEBUG[SCREEN::DISBLE]"));
...
Screen::leave()
{
	//LOG((CLOG_NOTE "DEBUG[SCREEN::LEAVE]"));
...
Screen::isLockedToScreen() const
{
	...
	UInt32 buttonID = 0;
	//LOG((CLOG_NOTE "DEBUG[SCREEN::isLockToScreen()][1]"));
	if (m_screen->isAnyMouseButtonDown(buttonID)) {

		//LOG((CLOG_NOTE "DEBUG[SCREEN::isLockToScreen()][2]"));
		
		if (buttonID != kButtonLeft) {
			LOG((CLOG_DEBUG "locked by mouse buttonID: %d", buttonID));
		}
		
		//LOG((CLOG_NOTE "DEBUG[SCREEN::isLockToScreen()][3]"));
		...
		if (m_enableDragDrop) {
			...
			//LOG((CLOG_NOTE "DEBUG[SCREEN::isLockToScreen()][4]"));
			return true;
		}
	}
