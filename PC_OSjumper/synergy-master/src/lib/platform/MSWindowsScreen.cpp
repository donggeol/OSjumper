...
MSWindowsScreen::leave()
{
	...
	//LOG((CLOG_NOTE "DEBUG[MSWINDOWSSCREEN::LEAVE]"));
	...
	m_isOnScreen = false;
	forceShowCursor();
	//LOG((CLOG_NOTE "DEBUG[1]"));
	if (isDraggingStarted() && !m_isPrimary) {

		//LOG((CLOG_NOTE "DEBUG[2]"));
...
MSWindowsScreen::sendDragThread(void*)
{
	...
	//LOG((CLOG_NOTE "m_draggingStarted[FALSE]"));
...
MSWindowsScreen::fakeMouseMove(SInt32 x, SInt32 y)
{
	...
	if (m_buttons[kButtonLeft]) {
		//LOG((CLOG_NOTE "m_draggingStarted[TRUE]"));
...
{
	//LOG((CLOG_NOTE "getDraggingFilename[1]"));
	if (m_draggingStarted) {
		...
		if (!filename.empty()) {
			if (DragInformation::isFileValid(filename)) {
				...
				LOG((CLOG_DEBUG "getDraggingFilename[4] : %s", filename.c_str()));
			}
			else {
