				...
				SInt32 x, SInt32 y, bool forScreensaver)
{
	...
	//LOG((CLOG_NOTE "DEBUG[SERVER::SWITCHSCREEN()][1]"));
	if (m_active != dst) {
		// leave active screen
		if (!m_active->leave()) {
			// cannot leave screen
			//LOG((CLOG_NOTE "DEBUG[SERVER::SWITCHSCREEN()][2]"));
		...
		// cut over
		m_active = dst;
		//LOG((CLOG_NOTE "m_active_CHANGE_switchScreen[2]"));
				...
				SInt32 xActive, SInt32 yActive)
{
	...
	// is there a neighbor?
	if (newScreen == NULL) {
		//LOG((CLOG_NOTE "DEBUG[SERVER::ISSWITCHOKAY][1]"));
	...
	//LOG((CLOG_NOTE "DEBUG[SERVER::ISSWITCHOKAY][2]"));
...
Server::onMouseMoveSecondary(SInt32 dx, SInt32 dy)
{
	...
	do {
		...
		if (m_x < ax) {
			...
			//LOG((CLOG_NOTE "DEBUG[SERVER::ONMOUSEMOVESECONDARY()][1]"));
		...
		if (!isSwitchOkay(newScreen, dir, m_x, m_y, xc, yc)) {
			//LOG((CLOG_NOTE "DEBUG[SERVER::ONMOUSEMOVESECONDARY()][2]"));
			newScreen = m_active;
			jump      = false;
		}
	} while (false);
	...
	if (jump) {
		// switch screens
		//LOG((CLOG_NOTE "DEBUG[SERVER::ONMOUSEMOVESECONDARY()][3]"));
...
Server::onFileChunkSending(const void* data)
{
	...
	//LOG((CLOG_NOTE "onFileChunkSending[1]"));
	assert(m_active != NULL);
	// relay
...
{
	//LOG((CLOG_NOTE "m_writeToDropDirThread Start"));
...
Server::writeToDropDirThread(void*)
{
	...
	//LOG((CLOG_NOTE "starting write to drop dir thread"));
...
Server::forceLeaveClient(BaseClientProxy* client)
{
	...
	if (active == client) {
		...
		// disconnected.
		LOG((CLOG_NOTE "jump from \"%s\" to \"%s\" at %d,%d", getName(active).c_str(), getName(m_primaryClient).c_str(), m_x, m_y));

		// cut over
		m_active = m_primaryClient;
		//LOG((CLOG_NOTE "m_active_CHANGE_forceLeaveClient[1]"));
...
Server::sendFileThread(void* data)
{
	try {
		...
		LOG((CLOG_NOTE "sending file to client, filename=%s", filename));
		FileChunker::sendFileChunks(filename, m_events, this);
	}
	catch (std::runtime_error error) {
		LOG((CLOG_NOTE "failed sending file chunks, error: %s", error.what()));
	}
