...
import org.synergy.io.msgs.DragInfoMessage;
import org.synergy.io.msgs.EnterMessage;
import org.synergy.io.msgs.FileTransferMessage;
...
public class ServerProxy {
    ...
    private static final double KEEP_ALIVE_RATE = 3.0; // 3.0;
	...
	protected Result parseMessage () throws IOException {
        ...
        case DCLIPBOARD:
			...
			break;
        case DFILETRANSFER:
        	fileTransfer(new FileTransferMessage (header, din));
        	break;
        case DDRAGINFO:
        	setDragInfo(new DragInfoMessage (header, din));
        	break;
		case CRESETOPTIONS:
			resetOptions (new ResetOptionsMessage (din));
			break;
	...
	private void grabClipboard (ClipboardMessage clipboardMessage) {
		client.clipBoard (clipboardMessage.toString());
	}

	private void setClipboard (ClipboardDataMessage clipboardDataMessage) {
		client.clipBoard (clipboardDataMessage.toString());
	}

	private void setDragInfo (DragInfoMessage dragInfoMessage) {
		client.dragInfo (dragInfoMessage.getFileName());
	}
	
	private void fileTransfer (FileTransferMessage fileTransferMessage) {
		client.fileTransfer (fileTransferMessage.getMark(), fileTransferMessage.getData());
	}
}
